...
contract ERC3525 is Context, IERC3525Metadata, IERC721Enumerable, Ownable {
    ...
    mapping(uint256 => mapping(address => mapping(address => uint256)))
        public _withdrawableFunds; // slotId=>assetOwner => (tokenAddress => funds)
    ...
    address private constant NATIVE_CURRENCY =
        0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE; //5000000000000000000
    ...
    function getTotalAssetTokensMinted(
        uint256 slotId
    ...
    function _setTokenURI(uint256 tokenId, string memory _tokenURI) internal {
        require(
            _exists(tokenId),
            "AssetRegistry: URI set of nonexistent token"
        );
    ...
    function setTokenURI(uint256 tokenId_) public view returns (string memory) {
        ...
        svgPart3 = string(
            abi.encodePacked(
                ...
                '</tspan><tspan x="120" y="280" text-anchor="end">',
                _allTokens[_allTokensIndex[tokenId_]].id.toString(),
                '</tspan><tspan x="120" y="300" text-anchor="end">',
                _allTokens[_allTokensIndex[tokenId_]].balance.toString(),
                '</tspan><tspan x="16" y="344" font-size="7">Owner:',
                getPartialAddress(ownerOf(tokenId_)),
                '</tspan><tspan x="16" y="356" font-size="7">Serial No</tspan><tspan x="516" y="380" font-size="7" fill="black" fill-opacity="0.6">Demo ASSET NFT </tspan></text></svg>'
            )
        );
    ...
    function getPurchasedAssets(
        ...
        require(
            user != address(0),
            "AssetRegistry: address zero is not a valid owner"
        );
    ...
    function getUserAssetTokenBalance(
        address user,
        uint256 slot
    ) public view returns (uint256) {
        require(
    ...
    function _addOwner(uint256 _assetId, address _owner) internal {
        ...
        if (
            _ownerIndices[_assetId][_owner] == 0 &&
            (_assetOwners[_assetId].length == 0 ||
                _assetOwners[_assetId][0] != _owner)
        ) {
    ...
    function approve(
        ...
        require(
            ...
            "AssetRegistry: approve caller is not owner nor approved"
        );
    ...
    function setAssetFractionalPriceUSD(
        uint256 _slotId,
        uint256 priceUSD
    ) public onlyAssetOwner(msg.sender, _slotId) {
        require(_slotExists(_slotId), "Asset doesn't exist");

        assetFractionalPriceUSD[_slotId] = priceUSD;
    }

    // Function to get the price of a given assetId in USD.
    function getAssetFractionalPriceUSD(
        uint256 _slotId
    ) public view returns (uint256) {
        require(_slotExists(_slotId), "LeasingContract: Nonexistent token");
    ...
    function transferFrom(
        ...
        require(
            fromSlot == toSlot,
            "Cannot transfer value between nfts of different assets"
        );
        _spendAllowance(_msgSender(), fromTokenId_, value_);
    ...
    function balanceOf(
        ...
        require(
            owner_ != address(0),
            "AssetRegistry: balance query for the zero address"
        );
    ...
    function transferFrom(
        ...
        require(
            _isApprovedOrOwner(_msgSender(), tokenId_),
            "AssetRegistry: transfer caller is not owner nor approved"
        );
    ...
    function tokenByIndex(
        ...
        require(
            ...
            "AssetRegistry: global index out of bounds"
        );
    ...
    function tokenOfOwnerByIndex(
        ...
        require(
            ...
            "AssetRegistry: owner index out of bounds"
        );
    ...
    function _spendAllowance(
        ...
        if (
            ...
            require(
                ...
                "AssetRegistry: insufficient allowance"
            );
    ...
    ) public virtual onlyAssetOwner(msg.sender, slot_) {
    ...
    function mintAsset(
        ...
        _withdrawableFunds[slot][assets[slot].owner][
            erc20TokenAddress
        ] += netAmount;
        ...
        if (
            erc20TokenAddress == NATIVE_CURRENCY && msg.value > amountToBePaid
        ) {
    ...
    function _burnValue(uint256 tokenId_, uint256 burnValue_) internal virtual {
        ...
        require(
            value >= burnValue_,
            "AssetRegistry: burn value exceeds balance"
        );
    ...
    function _approveValue(
        ...
        require(
            to_ != address(0),
            "AssetRegistry: approve value to the zero address"
        );
    ...
    function _transferValue(
        ...
        require(
            ...
            "AssetRegistry: transfer from invalid token ID"
        );
        require(
            _exists(toTokenId_),
            "AssetRegistry: transfer to invalid token ID"
        );

        AssetData storage fromAssetData = _allTokens[
        ...
        require(
            ...
            "AssetRegistry: insufficient balance for transfer"
        );
        require(
            fromAssetData.slot == toAssetData.slot,
            "AssetRegistry: transfer to token with different slot"
        );
    ...
    function _transferTokenId(
        ...
        require(
            ...
            "AssetRegistry: transfer from invalid owner"
        );
        require(
            to_ != address(0),
            "AssetRegistry: transfer to the zero address"
        );
    ...
    function withdrawFunds(
        ...
        address erc20TokenAddress,
        uint256 amount
    ) public onlyAssetOwner(msg.sender, slotId) {
        uint256 availableFunds = _withdrawableFunds[slotId][msg.sender][
            erc20TokenAddress
        ];
